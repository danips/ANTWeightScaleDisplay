...
public class Decode {
   ...
   private static class AccumulatedField {
      final int mesgNum;
      final int destFieldNum; //Field# to accumulate into
      long lastValue;
   ...
   private static class Accumulator {
      ...
      final ArrayList<AccumulatedField> accumulatedFields;

      Accumulator() {
         accumulatedFields = new ArrayList<>();
      }
   ...
   private final MesgDefinition[] localMesgDefs = new MesgDefinition[Fit.MAX_LOCAL_MESGS];
   private final HashMap<Short, DeveloperDataIdMesg> developerDataIds = new HashMap<>();
   private final HashMap<Short, HashMap<Short, FieldDescriptionMesg>> developerFields = new HashMap<>();

   private int numFields;
   private int fieldIndex;
   private int fieldDataIndex;
   private int fieldBytesLeft;
   private final byte[] fieldData = new byte[Fit.MAX_FIELD_SIZE];
   ...
   private final ArrayList<MesgDefinitionListener> mesgDefListeners = new ArrayList<>();
   private final ArrayList<DeveloperFieldDescriptionListener> devFieldDescListeners = new ArrayList<>();
   ...
   public Decode() {
      ...
      if (Fit.debug)
         System.out.print("Fit.Decode: Starting decode...\n");
   }
                  ...
                  case MESG:
                     switch (mesg.num) {
                        case MesgNum.DEVELOPER_DATA_ID: {
                           ...
                           developerFields.put(index, new HashMap<>());
                           break;
                        }
   ...
   public RETURN read(byte data) {
      ...
      switch (state) {
            ...
            if (fileBytesLeft > 1) {
               if ((data & Fit.HDR_TIME_REC_BIT) != 0) {
                  ...
                  if ((data & Fit.HDR_TYPE_DEF_BIT) != 0) {
                     ...
                     hasDevData = (data & Fit.HDR_DEV_FIELDS_BIT) != 0;
         ...
         case DEV_FIELD_DEV_ID:
            ...
            if(developerFields.containsKey((short) data)) {
               // There is a Developer ID Message for this Field
               fieldDefinition.setDeveloperDataIdMesg(developerDataIds.get((short) data));

               if(developerFields.get((short) data).containsKey(fieldDefinition.getNum())) {
                  // The Developer has Defined this field number
                  FieldDescriptionMesg fieldDescription = developerFields.get((short) data).get(fieldDefinition.getNum());
