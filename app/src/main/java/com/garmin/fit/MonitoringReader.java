...
public class MonitoringReader implements MonitoringInfoMesgListener,
   ...
   *
   */
   private static class ExtractState {
   ...
   private final ArrayList<MonitoringMesgListener> listeners;
   ...
   private final TreeMap<ActivityType, ArrayList<MonitoringMesg>> intervalMesgs;
   private final TreeMap<ActivityType, MonitoringMesg> lastMesgs;
   ...
   private final HashSet<String> summedInstantaneousFields;
   private TreeMap<ActivityType, ExtractState> extractStates;
   private final HashMap<String, Boolean> includedFields;
   ...
   public MonitoringReader(int interval) {
      ...
      intervalMesgs = new TreeMap<>();
      lastMesgs = new TreeMap<>();
      localTimeOffset = 0;
      systemToUtcTimestampOffset = 0;
      systemToLocalTimestampOffset = 0;
      extractStates = new TreeMap<>();
      includedFields = new HashMap<>();
      summedInstantaneousFields = new HashSet<>();
       Collections.addAll(summedInstantaneousFields, summedInstantaneousFieldNames);
   ...
   public HashMap<String, Boolean> getFieldStates() {
      return new HashMap<>(includedFields);
   }
   ...
   private boolean broadcastInterval(boolean broadcastCompleteIntervalsOnly) {
      Iterator<ActivityType> activityTypeIterator = intervalMesgs.keySet()
            .iterator();
      TreeMap<ActivityType, MonitoringMesg> broadcastMesgs = new TreeMap<>();
   ...
   private void setFieldsFromMesg(MonitoringMesg destMesg,
       ...
       for (Field field : srcMesg.fields) destMesg.setField(new Field(field));
   }
   ...
   private MonitoringMesg computeInterval(ActivityType activityType,
      ...
      ArrayList<ReaderField> fields = new ArrayList<>();
      ...
      for (MonitoringMesg mesg : intervalMesgs) {
         boolean updateMesg = true;
          for (String summedInstantaneousFieldName : summedInstantaneousFieldNames) {
              if (mesg.getField(summedInstantaneousFieldName) != null) {
                  updateMesg = false;
   ...
   private interface ReaderField extends MonitoringMesgListener {
      boolean setMesg(MonitoringMesg mesg);
      double getVal();
      String getName();
   }
